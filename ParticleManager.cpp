...
XMFLOAT3 ParticleManager::eye = { 5.0f, 0, 5.0f };
...
void ParticleManager::InitializeGraphicsPipeline()
{
	...
	ComPtr<ID3DBlob> gsBlob;	// ジオメタリシェーダオブジェクト
...
void ParticleManager::LoadTexture()
{
	...
	// WICテクスチャのロード
	result = LoadFromWICFile(L"Resources/dog.png", WIC_FLAGS_NONE, &metadata, scratchImg);
	assert(SUCCEEDED(result));
...
#pragma region Y軸回りビルボード行列の計算
	...
	// Z軸はX軸-＞Y軸の外積で求まる
	ybillcameraAxisZ = XMVector3Dot(ybillcameraAxisX, ybillcameraAxisY);
...
/// パーティクルの追加
/// </summary>
void ParticleManager::Add(int life, XMFLOAT3 position, XMFLOAT3 verosity, XMFLOAT3 accel) {
	// list に要素を追加
